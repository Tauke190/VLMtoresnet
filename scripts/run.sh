...
CUDA_VISIBLE_DEVICES=0,1 python -m torch.distributed.launch --nproc_per_node=$NUM_GPU --master_port=$PORT train_baseline.py \
    ...
    --validation-eval-interval 1 --initial-checkpoint $WT --output $OUTPUT --experiment $EXP --method attention_distillation \
    --freeze-backbone --native-amp --workers 12 \
    -b 32 --lr 1e-3 --drop-path 0.35 --mixup 0 --cutmix 0 --epochs 50 --input-size 3 224 224 --log-wandb
