...
# ----- Noise experiment options (NEW) -----
parser.add_argument(
    "--noise-experiment",
    action="store_true",
    default=False,
    help="Run noise robustness experiment instead of standard training",
)
parser.add_argument(
    "--noise-mean-var",
    type=float,
    default=0.0033,
    help="Mean variance of model weights used to scale added noise.",
)
parser.add_argument(
    "--noise-step",
    type=float,
...
parser.add_argument(
    ...
    type=float,
    default=1.0,
    help="Max noise as fraction of mean variance (1.0 = 100%%).",
)
parser.add_argument(
    "--recovery-acc",
    type=float,
    default=80.0,
    help="Top-1 accuracy threshold to consider the model recovered.",
)
parser.add_argument(
    "--noise-max-epochs",
    type=int,
...
parser.add_argument(
    ...
    type=int,
    default=50,
    help="Number of validation batches to use as subset during noise experiment.",
)
...
def main():
    ...
    model_ema = None
    if args.model_ema and not args.noise_experiment:  # MODIFIED
    ...
    # ...existing code that defines:
    #   optimizer, amp_autocast, loss_scaler, model_ema, lr_scheduler, wd_scheduler,
    #   loader_train, loader_eval, train_loss_fn, validate_loss_fn, saver, output_dir, etc.

    if args.noise_experiment:
        # Run noise robustness study instead of standard training
...
def run_noise_experiment(
    ...
    """
    For each noise level (5%, 10%, ... of mean variance), add Gaussian noise to weights,
    fine-tune, and record time until top-1 >= args.recovery_acc using a validation subset.
    Produces noise_recovery.png with:
        x-axis: noise level (% of mean variance)
        y-axis: time to recover (hours)
    """
    # Save clean baseline weights
    baseline_state = {
        k: v.detach().cpu().clone() for k, v in model.state_dict().items()
    }

    if args.local_rank == 0:
        _logger.info("Evaluating baseline model (subset) before adding noise...")
    baseline_metrics = validate(
        ...
        args,
        amp_autocast=amp_autocast,
        max_batches=args.noise_val_batches,
    )
    baseline_top1 = baseline_metrics["top1"]
    if args.local_rank == 0:
        _logger.info(f"Baseline subset top1 accuracy: {baseline_top1:.3f}")

    noise_factors_pct = []  # x-axis
    recovery_hours = []     # y-axis

    max_epochs = args.noise_max_epochs or args.epochs
    ...
    while noise_factor <= args.noise_max + 1e-8:
        ...
        if args.local_rank == 0:
            _logger.info(
                ...
                f"(mean_var={args.noise_mean_var}) ====="
            )

        # Restore clean baseline weights
        ...
        # Fresh optimizer & schedulers per noise level
        optimizer = create_optimizer_v2(model, **optimizer_kwargs(cfg=args))
        ...
        # Start timing from first noisy eval
        t_start = time.time()

        # Initial evaluation after adding noise (on subset)
        eval_metrics = validate(
            model,
            loader_eval,
            validate_loss_fn,
        ...
        if args.local_rank == 0:
            _logger.info(
                f"Initial noisy subset top1: {top1:.3f} "
                f"(target >= {args.recovery_acc:.1f})"
            )
        ...
        while (not recovered) and (epoch < max_epochs):
            ...
            for batch_idx, (inputs, targets) in enumerate(loader_train):
                ...
                with amp_autocast():
                    ...
                    loss = train_loss_fn(inputs, outputs, targets)
                ...
                # Validate on subset every N batches
                if num_updates % val_interval == 0:
                    ...
                    if args.local_rank == 0:
                        _logger.info(
                            ...
                            f"update {num_updates}, epoch {epoch}, batch {batch_idx}: "
                            f"subset top1={top1:.3f}"
                        )
                    if top1 >= args.recovery_acc:
                        recovered = True
                        break

            # Optional final check at end of epoch if not yet recovered
            if not recovered:
                eval_metrics = validate(
                    model,
                    loader_eval,
                ...
                if args.local_rank == 0:
                    _logger.info(
                        f"[Noise {noise_pct:.1f}%%] "
                        f"end of epoch {epoch}: subset top1={top1:.3f}"
                    )
                if top1 >= args.recovery_acc:
                    recovered = True
                    break
        ...
        if args.local_rank == 0:
            if recovered:
                _logger.info(
                    f"Recovered (subset top1 >= {args.recovery_acc:.1f}) at "
                    f"noise {noise_pct:.1f}% in {elapsed_hours:.3f} hours."
                )
            else:
                _logger.info(
    ...
    # Plot: noise% vs time to recover (hours)
    if args.local_rank == 0:
        plt.figure(figsize=(6, 4))
        plt.plot(noise_factors_pct, recovery_hours, marker="o")
        plt.xlabel("Noise level (% of mean variance)")
        plt.ylabel("Time to recover (hours)")
        plt.title(f"{args.model} noise recovery (subset, target ≥ {args.recovery_acc:.1f})")
        plt.grid(True)

        out_dir = output_dir or "."
        os.makedirs(out_dir, exist_ok=True)
        out_path = os.path.join(out_dir, "noise_recovery.png")
