...
import sys
import os
from scaleKDUtils import get_resnet50_prepool_features , CNNtoViTTokenDistiller
# --- Configuration ---
...
def get_teacher_tokens(model, images):
    with torch.no_grad():
        # images: [B, 3, 224, 224]
        # model.visual is the ViT
        x = model.visual.conv1(images)  # shape: [B, width, grid, grid]
        ...
        class_emb = class_emb + torch.zeros(x.shape[0], 1, x.shape[2], device=x.device, dtype=x.dtype)
        x = torch.cat([class_emb, x], dim=1)  # prepend class token
        x = x + model.visual.positional_embedding.to(x.dtype)
        x = model.visual.ln_pre(x)
        x = x.permute(1, 0, 2)  # NLD -> LND for transformer
        x = model.visual.transformer(x)
        x = x.permute(1, 0, 2)  # LND -> NLD
        # x: [B, num_tokens, width]
        return x  # all tokens (class + patch tokens)
