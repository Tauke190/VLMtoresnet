...
def main():
    ...
    # load checkpoint manually, skipping the head
    checkpoint = load_checkpoint(runner.model, args.checkpoint, map_location='cpu')
    # Remove the head if it causes size mismatch
    state_dict = {k: v for k, v in checkpoint['state_dict'].items() if 'head' not in k}
