...
# Datasets to evaluate (currently only SST2 enabled)
DATASETS = [
    dict(
        name="fgvc_aircraft",
    ...
    dict(
        ...
        # ./classes/gtsrb.txt, one label per line, e.g.
...
def _read_txt_lines(root_dir: str, name: str, required: bool):
...
def load_templates(name: str):
    """Load text prompt templates for zero-shot classification."""
    return _read_txt_lines(TEMPLATE_DIR, name, required=True)


def load_classnames(name: str):
...
def get_classnames_from_dataset(dataset):
    """Infer class names from a dataset object (torchvision-style or label array)."""
    # 1) Standard torchvision-style
    ...
    # 2) Fallback: infer from labels/targets and use string IDs
    labels = None
    ...
    raise ValueError(
        ...
        "please provide a classes file and set classes_name for this dataset."
    )


@torch.no_grad()
def build_zeroshot_classifier(model, tokenizer, classnames, templates, device):
    """Build EVA02 text embeddings for each class using prompt templates."""
    weights = []
...
def eval_dataset(
    ...
    """Evaluate zero-shot EVA02 on a single dataset."""
    print(f"\n=== {name} ===")
    ...
    if classes_name:
        ...
        if classnames:
            print(f"Using classes from: {os.path.join(CLASSES_DIR, classes_name)}")
        else:
            print(
                f"Warning: class file {classes_name} not found, will try dataset metadata."
            )
    ...
    if classnames is None:
        try:
            ...
            print("Using classes from dataset")
    ...
    # Build text classifier and evaluate
    weights = build_zeroshot_classifier(model, tokenizer, classnames, templates, device)
...
def main():
    """Run zero-shot EVA02 evaluation over all configured datasets."""
    print(f"Using EVA02 model: {MODEL_ARCH} ({PRETRAINED}) on {DEVICE}")
    model, _, preprocess = open_clip.create_model_and_transforms(
        MODEL_ARCH, pretrained=PRETRAINED, device=DEVICE
    )
