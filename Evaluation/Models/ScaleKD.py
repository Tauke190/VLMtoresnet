...
from torch.utils.data import DataLoader  # unused but kept for symmetry with ConvMixer
...
def build_resnet50d():
    """Build ResNet-50-D student as an MMPretrain ImageClassifier."""
    cfg = dict(
...
def load_student_ckpt(model: torch.nn.Module, ckpt_path: str):
    ...
    if any(k.startswith(("backbone.", "head.")) for k in state.keys()):
        ...
        print("=> load_state_dict (mmcls style) missing:", msg.missing_keys)
...
# Standard ImageNet transform (same as ConvMixer script: 224x224)
...
def get_features(model, dataloader, dataset_name, split):
    """Extract and cache features from ScaleKD student."""
    ...
    with torch.no_grad():
        for images, labels in tqdm(dataloader, desc=f"  {split}"):
            # Backbone + neck to get pooled features
...
def evaluate_dataset(dataset_name, loader_fn, skip_download: bool = False):
    """Evaluate ScaleKD student on a single dataset (ConvMixer-style pipeline)."""
    print(f"\n{'='*70}")
    ...
    try:
        ...
        print(f"\n  Error on {dataset_name}: {str(e)}")
        return None


# Dataset configurations (same structure as ConvMixer script)
datasets_config = [
...
# Print summary
print(f"\n\n{'='*70}")
print("SCALEKD ResNet-50-D - LINEAR PROBING RESULTS SUMMARY")
print(f"{'='*70}")
print(
...
for name, res in results.items():
    print(
        f"{name:<20} {res['train_size']:>8} {res['test_size']:>8} "
